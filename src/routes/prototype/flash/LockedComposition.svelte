<!--
	Locked composition for the flash prototype — a VERBATIM local copy of
	../palette/LockedComposition.svelte. THROWAWAY — do not fold into main.
	Answers question #15 (session-end flash mechanism): flash prototype copies the
	locked composition; changes go only in the flash layer, never in this
	composition component. The LOCKED layout is the one decided in ticket #7
	(centered-hero timer SECTION on top, dense task TABLE below), painted in the
	decided coral palette (issue #14). This component reads palette tokens through
	Tailwind arbitrary values like bg-[var(--surface)] and the ff-display /
	ff-heading helper classes. State-color wiring — hover / active (focus-ring) /
	error / success — is demonstrated on real interactive states, unchanged from the
	palette prototype.
-->
<script lang="ts">
