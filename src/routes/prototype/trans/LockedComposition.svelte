<!--
	Locked composition for the trans prototype — a VERBATIM local copy of
	../flash/LockedComposition.svelte with ONE change: the `transitioning` branch
	no longer hardcodes a placeholder prompt card; it renders `{@render children?.()}`
	so the host page can swap any of the four prompt variants (VariantA..D) in via
	a snippet. THROWAWAY — do not fold into main.

	Answers #16 (transitioning prompt micro-UX): this composition is the LOCKED
	layout decided in #7 (centered-hero timer SECTION on top, dense task TABLE
	below), painted in the decided coral palette (#14, boldened by #15) and
	sitting under #15's held palette swap (`data-phase` flips coral → sage at
	focus-end, so the prompt renders ON the sage break palette). State-colour
	wiring — hover / active (focus-ring) / error / success — unchanged.

	The static 0:00 focus ring is INTENTIONALLY kept visible during the
	`transitioning` state: variants A/C/D use it as "the focus just ended"
	context behind/around their prompt; variant B overlays its own 30s countdown
	ring on top to "replace" it. Keeping the ring lets each variant take a real
	structural position on the countdown-shape axis without touching the locked
	composition's hero markup.
-->
<script lang="ts">
	import type { Snippet } from 'svelte';
	import type { Task, TimerScene } from './demo.svelte.ts';
	import { pips, primaryTask, sceneMeta } from './demo.svelte.ts';
	...
	let { scene, tasks, children }: Props = $props();
...
<main class="mx-auto max-w-5xl px-6 py-16">
	...
	<section class="text-center">
		...
		<!-- Timer hero — `relative` so the transitioning prompt (rendered as the
		     children snippet below) can absolute-position over the hero. -->
		<div class="relative mt-12 flex flex-col items-center">
			...
			{#if isTransition}
				<!-- Prompt slot — the host page supplies the variant (VariantA..D) as
				     the children snippet. Each variant owns its own absolute layout
				     over this `relative` hero container. The static 0:00 ring above is
				     intentionally kept as context (B overlays its own ring on top). -->
				{@render children?.()}
			{:else}
